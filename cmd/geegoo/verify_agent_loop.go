...
func runVerifyAgentLoop(args []string) {
	...
	offline := fs.Bool("offline", false, "run parity checks without config.json (stub registry only)")
	if err := fs.Parse(args); err != nil {
		os.Exit(2)
	}

	var cards []verify.AgentLoopCard
	if *offline {
		cards = verify.VerifyAgentLoopParity(verify.OfflineAgentLoopRegistry(""))
		fmt.Println("Mode: offline (no config)")
	} else {
