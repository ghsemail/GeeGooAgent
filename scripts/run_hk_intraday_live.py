#!/usr/bin/env python3
"""Run HK GRID intraday decision: pick a real GRID bot or fallback to 00700.HK."""
...
def pick_hk_grid_bot(mcp_token: str) -> dict:
    remote = f"""
import json
from bson import ObjectId
from pymongo import MongoClient
...
prefer = [r for r in rows if r.get("code") == "00700.HK"]
...
"""
    ...
    return {
        "code": bot.get("code") or FALLBACK["code"],
        "stock_name": bot.get("stock_name") or FALLBACK["stock_name"],
        "bot_id": str(bot.get("bot_id") or FALLBACK["bot_id"]),
        "bot_name": bot.get("botname") or bot.get("bot_name") or FALLBACK["bot_name"],
        "bot_type": "GRID",
...
def verify_report(mcp_token: str, code: str, bot_id: str) -> str:
    remote = f"""
import json, urllib.request
mcp_token = {json.dumps(mcp_token)}
api_key = {json.dumps(BOT_KEY)}
body = {{"mcp_token": mcp_token, "code": {json.dumps(code)}, "bot_id": {json.dumps(bot_id)}, "limit": 5}}
req = urllib.request.Request(
...
def main() -> int:
    ...
    print(verify_report(mcp_token, sim["code"], sim["bot_id"]))
    return 0
