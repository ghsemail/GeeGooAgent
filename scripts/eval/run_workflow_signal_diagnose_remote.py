#!/usr/bin/env python3
"""Upsert workflow eval cases and run signal_diagnose live eval on geegoo-agent."""
...
def main() -> int:
    ...
    sftp.put(
        str(ROOT / "run_workflow_signal_diagnose_live.py"),
        f"{REMOTE_DIR}/run_workflow_signal_diagnose_live.py",
    )
    sql_path = f"{REMOTE_DIR}/workflow_eval_seed.sql"
    sftp.put(str(ROOT / "workflow_eval_seed.sql"), sql_path)
    sftp.close()
    ...
    cmd = f"cd {REMOTE_DIR} && python3 run_workflow_signal_diagnose_live.py 2>&1"
    if len(sys.argv) > 1:
