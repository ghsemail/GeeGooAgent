...
func AllHTTP() []HTTPSpec {
	raw := []HTTPSpec{
		...
		{Name: "diagnose_bot_signal_series", Description: "诊断单指标/规则链为何未触发信号（基于 K 线重算指标并按规则解释）。用户追问「为什么没信号/怎么算的/阈值有没有碰到」时调用；**不要**重复 probe 测次数。返回 param、algorithm、指标 min/max/last、last_bar/near_miss、sample_reasons、summary。必填 code、frequency、buy_signal；可选 sell_signal、months_back、side(buy/sell/both)。", Path: "/diagnoseBotSignalSeries", DirectResponse: true, MergePayload: true, Parameters: diagnoseBotSignalSeriesParameters()},
		{Name: "run_strategy_backtest", Description: "catalog 信号策略回测（probe+模拟+落库，与 trading_operation「回测运行」一致）。用户要「跑回测/看收益/回撤/成交笔数/验证策略」且未明确 DCA/网格/定投时**必须调用本工具**；SAR+MACD 等组合信号回测也走此路径。返回 log_id、profit_rate、final_value、trade_count。必填 code、frequency、buy_signal；可选 sell_signal、strategy_label、period(默认 3m)、fund(默认100000)、months_back(默认 3)、trade_config。勿用于 DCA/Grid Bot 方案——那走 generate_* + loopback_strategy。", Path: "/runStrategyBacktest", DirectResponse: true, MergePayload: true, Parameters: runStrategyBacktestParameters()},
