...
func AllHTTP() []HTTPSpec {
	raw := []HTTPSpec{
		...
		{Name: "list_strategy_backtest_logs", Description: "列出策略回测历史（trading_operation 回测页保存的记录）。可按 code、strategy_label、日期筛选；返回摘要含 profit、profit_rate、trade_count、has_chart_data。", Path: "/listStrategyBacktestLogs", DirectResponse: true, MergePayload: true, Parameters: listStrategyBacktestLogsParameters()},
