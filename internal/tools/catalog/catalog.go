...
func AllHTTP() []HTTPSpec {
	raw := []HTTPSpec{
		...
		{Name: "run_strategy_backtest", Description: "catalog 信号策略回测（probe+模拟+落库，与 trading_operation「回测运行」一致）。用户要「跑回测/看收益/回撤/成交笔数/验证策略」且未明确 DCA/网格/定投时**必须调用本工具**；SAR+MACD 等组合信号回测也走此路径。返回 log_id、profit_rate、final_value、trade_count。必填 code、frequency、buy_signal；可选 sell_signal、strategy_label、period(1m/3m)、fund(默认100000)、months_back、trade_config。勿用于 DCA/Grid Bot 方案——那走 generate_* + loopback_strategy。", Path: "/runStrategyBacktest", DirectResponse: true, MergePayload: true, Parameters: runStrategyBacktestParameters()},
