...
func AllHTTP() []HTTPSpec {
	raw := []HTTPSpec{
		...
		{Name: "run_strategy_backtest", Description: "运行高级策略回测（probe + SmartTrade 模拟 + 写入 strategy_backtest_log）。用户要「回测」「跑回测」「验证策略收益」时优先调用本工具，勿仅用 probe_bot_signal_series。返回 log_id、profit_rate、final_value。默认 trade_config 与 trading_operation 回测页一致。", Path: "/runStrategyBacktest", DirectResponse: true, MergePayload: true, Parameters: runStrategyBacktestParameters()},
