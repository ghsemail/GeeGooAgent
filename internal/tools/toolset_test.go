...
func TestDefaultChatIncludesAllToolsetsExceptRecall(t *testing.T) {
	...
	if len(names) != 109 {
		t.Fatalf("default chat allowlist want 109, got %d", len(names))
	}
}
...
func TestToolsetCountsMatchDocumentation(t *testing.T) {
	...
	want := map[string]int{
		...
		"strategy": 10, "trading_bot": 15, "hedge_bot": 5, "reminder_manager": 15,
	...
	if len(union) != 111 {
		t.Fatalf("toolset union want 111, got %d", len(union))
	}
}
