...
func TestDefaultChatIncludesAllToolsetsExceptRecall(t *testing.T) {
	...
	if len(names) != 110 {
		t.Fatalf("default chat allowlist want 110, got %d", len(names))
	}
}
...
func TestToolsetCountsMatchDocumentation(t *testing.T) {
	...
	want := map[string]int{
		...
		"strategy": 11, "trading_bot": 15, "hedge_bot": 5, "reminder_manager": 15,
	...
	if len(union) != 112 {
		t.Fatalf("toolset union want 112, got %d", len(union))
	}
}
