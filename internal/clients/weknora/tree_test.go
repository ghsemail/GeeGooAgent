...
func TestBuildFolderTree(t *testing.T) {
	...
	tree := BuildFolderTree([]Document{
		...
		{FileName: "macd.pdf", FolderPath: "策略资料"},
		{FileName: "a.md", FolderPath: "策略资料/子目录"},
		{FileName: "b.md", FolderPath: "策略资料/子目录"},
	})
	...
	if len(tree.Folders) != 1 || tree.Folders[0].Path != "策略资料" {
	...
	if len(tree.Folders[0].Children) != 1 || tree.Folders[0].Children[0].Path != "策略资料/子目录" {
