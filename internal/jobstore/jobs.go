...
// RecordSkillVerdict updates last_run/last_verdict for scheduler jobs matching skill+market.
// Manual backfills and API-triggered runs use this so health checks stay in sync.
func RecordSkillVerdict(dir, skill, market, verdict string) error {
	...
	for i := range jf.Jobs {
		...
		jobMarket := strings.ToUpper(strings.TrimSpace(j.Market))
		if market != "" {
			if jobMarket != market {
				continue
			}
		...
		j.LastRun = now
		j.LastVerdict = verdict
		updated = true
	}
	if !updated {
		return nil
	}
	return SaveJobs(dir, jf)
}
