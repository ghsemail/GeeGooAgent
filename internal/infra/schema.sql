...
(
    'workflow_strategy_dev_read_cognition', '', 'Workflow · 策略开发读档案（Macd4H）',
...
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_dca_grid_backtest', '', 'TurnPlan · 单轮 · DCA 定投回测', '独立 session：DCA/网格类回测意图；若 Agent clarify 由 auto-clarify 自动应答。', '["每次运行新开 Chat 会话（评估日志与 session_id 仍会保存）","单轮发送：「帮我做一个DCA定投策略回测」","若 Agent clarify：由 auto-clarify 根据对话上下文自动应答","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"帮我做一个DCA定投策略回测","expect_domain":"dca_grid","expect_mode":"execute","expect_sop":false,"require_tools":["generate_dca_strategy"],"min_reply_chars":20,"turn_id":"dca_grid_backtest","turn_plan_group":"backtest","dialogue":[{"role":"user","text":"帮我做一个DCA定投策略回测","judge":true}],"expect_reply":{"rubric":"应识别 DCA/定投回测意图并给出方案、参数确认或回测执行说明。","must_cover":["DCA"]},"expect_intent":{"domain":"dca_grid","mode":"execute","sop":false},"expect_execution":{"profile":"","legacy_require_tools":["generate_dca_strategy"]},"expect_routing":{"domain":"dca_grid","mode":"execute","sop":false},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7},"auto_clarify_only":true}', 75, 1, datetime('now'), datetime('now'));
