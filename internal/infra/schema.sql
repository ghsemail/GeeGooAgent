...
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_signal_list_then_probe', '', 'TurnPlan · 多轮 · 分析腾讯后选策略回测', '同 session：先分析腾讯价格走势，再问适合策略，最后发起回测。', '["新 session：运行前清空 Dock Chat","同 session 按序发送：「帮我分析一下腾讯的价格走势」 → 「帮我看看哪些策略适合腾讯」 → 「帮我用这些策略回测一下」","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"帮我用这些策略回测一下","setup_messages":["帮我分析一下腾讯的价格走势","帮我看看哪些策略适合腾讯"],"expect_domain":"backtest_run","expect_mode":"execute","expect_sop":false,"forbid_tools":["probe_bot_signal_series"],"require_tools":["run_strategy_backtest"],"min_reply_chars":20,"turn_id":"signal_list_then_probe","dialogue":[{"role":"user","text":"帮我分析一下腾讯的价格走势"},{"role":"user","text":"帮我看看哪些策略适合腾讯"},{"role":"user","text":"帮我用这些策略回测一下","judge":true}],"expect_reply":{"rubric":"在已分析腾讯并讨论适合策略后，应识别回测意图并针对腾讯发起或汇报回测进度/结果，而非只做静态分析或 probe 买卖点。","must_cover":["腾讯","回测"]},"expect_intent":{"domain":"backtest_run","mode":"execute","sop":false},"expect_execution":{"profile":"","forbid_tools":["probe_bot_signal_series"],"legacy_require_tools":["run_strategy_backtest"]},"expect_routing":{"domain":"backtest_run","mode":"execute","sop":false,"forbid_tools":["probe_bot_signal_series"]},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7}}', 56, 1, datetime('now'), datetime('now'));
