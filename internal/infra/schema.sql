...
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_ambiguous_bare_macd', '', 'TurnPlan · 单轮 · 模糊 MACD', '独立 session：无上下文的 MACD 用法问题，应 clarify；clarify 工具回调默认选「先问答」。', '["新 session：运行前清空 Dock Chat","单轮发送：「这个MACD信号平时该怎么用比较好」","若 Agent clarify：自动回复「先问答，先不操作」","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"这个MACD信号平时该怎么用比较好","expect_domain":"ambiguous","expect_mode":"clarify","expect_sop":false,"forbid_tools":["run_strategy_backtest","probe_bot_signal_series"],"clarify_reply":"先问答，先不操作","min_reply_chars":20,"turn_id":"ambiguous_bare_macd","dialogue":[{"role":"user","text":"这个MACD信号平时该怎么用比较好","judge":true}],"expect_reply":{"rubric":"应对模糊的 MACD 信号问题给出澄清选项（测点/回测/讲解指标等），不应直接执行回测或测点工具。","must_not":["回测已完成","开始回测"]},"expect_intent":{"domain":"ambiguous","mode":"clarify","sop":false},"expect_execution":{"profile":"","forbid_tools":["run_strategy_backtest","probe_bot_signal_series"]},"expect_routing":{"domain":"ambiguous","mode":"clarify","sop":false,"forbid_tools":["run_strategy_backtest","probe_bot_signal_series"]},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7}}', 62, 1, datetime('now'), datetime('now'));
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_compound_analysis_backtest', '', 'TurnPlan · 单轮 · 分析+回测复合', '独立 session：一句话含分析与回测，应澄清而非直接执行；clarify 工具回调默认选「先分析」。', '["新 session：运行前清空 Dock Chat","单轮发送：「帮我把中际旭创分析一下，然后再跑个回测看看效果」","若 Agent clarify：自动回复「个股/指标分析」","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"帮我把中际旭创分析一下，然后再跑个回测看看效果","expect_domain":"ambiguous","expect_mode":"clarify","expect_sop":false,"forbid_tools":["run_strategy_backtest"],"clarify_reply":"个股/指标分析","min_reply_chars":20,"turn_id":"compound_analysis_backtest","dialogue":[{"role":"user","text":"帮我把中际旭创分析一下，然后再跑个回测看看效果","judge":true}],"expect_reply":{"rubric":"一句话同时含分析与回测时，应澄清用户想先做哪一步或分步确认，不应未经确认直接全套执行。"},"expect_intent":{"domain":"ambiguous","mode":"clarify","sop":false},"expect_execution":{"profile":"","forbid_tools":["run_strategy_backtest"]},"expect_routing":{"domain":"ambiguous","mode":"clarify","sop":false,"forbid_tools":["run_strategy_backtest"]},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7}}', 63, 1, datetime('now'), datetime('now'));
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_stock_quote_ambiguous', '', 'TurnPlan · 单轮 · 股价灰区', '独立 session：口语股价问法应 clarify；clarify 工具回调默认选「只要当前价」。', '["新 session：运行前清空 Dock Chat","单轮发送：「腾讯股价怎么样」","若 Agent clarify：自动回复「只要当前价」","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"腾讯股价怎么样","expect_domain":"ambiguous","expect_mode":"clarify","expect_sop":false,"forbid_tools":["run_strategy_backtest","get_mcp_analysis"],"clarify_reply":"只要当前价","min_reply_chars":20,"turn_id":"stock_quote_ambiguous","dialogue":[{"role":"user","text":"腾讯股价怎么样","judge":true}],"expect_reply":{"rubric":"对模糊的股价问法应给出澄清选项（只要当前价 / 分析价格走势），不应未经确认直接跑 MCP 分析或回测。","must_cover":["腾讯"],"must_not":["开始回测"]},"expect_intent":{"domain":"ambiguous","mode":"clarify","sop":false},"expect_execution":{"profile":"","forbid_tools":["run_strategy_backtest","get_mcp_analysis"]},"expect_routing":{"domain":"ambiguous","mode":"clarify","sop":false,"forbid_tools":["run_strategy_backtest","get_mcp_analysis"]},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7}}', 64, 1, datetime('now'), datetime('now'));
