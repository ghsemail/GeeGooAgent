...
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_ambiguous_bare_macd', '', 'TurnPlan · 两轮 · 模糊 MACD', '独立 session：首轮应 clarify；用户选择「先问答」后应讲解 MACD 用法。', '["新 session：运行前清空 Dock Chat","首轮发送：「这个MACD信号平时该怎么用比较好」","若 Agent 展示澄清选项，用户选择：「先问答，先不操作」","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"这个MACD信号平时该怎么用比较好","expect_domain":"ambiguous","expect_mode":"clarify","expect_sop":false,"forbid_tools":["run_strategy_backtest","probe_bot_signal_series"],"clarify_reply":"先问答，先不操作","min_reply_chars":20,"turn_id":"ambiguous_bare_macd","dialogue":[{"role":"user","text":"这个MACD信号平时该怎么用比较好"},{"role":"user","text":"先问答，先不操作","judge":true,"on_clarify":true}],"expect_reply":{"rubric":"用户已选择「先问答，先不操作」后，应通俗讲解 MACD 信号的日常用法或注意事项，走知识问答而非测点/回测。","must_cover":["MACD"],"must_not":["回测已完成","开始回测"]},"expect_intent":{"domain":"ambiguous","mode":"clarify","sop":false},"expect_execution":{"profile":"","forbid_tools":["run_strategy_backtest","probe_bot_signal_series"]},"expect_routing":{"domain":"ambiguous","mode":"clarify","sop":false,"forbid_tools":["run_strategy_backtest","probe_bot_signal_series"]},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7}}', 62, 1, datetime('now'), datetime('now'));
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_compound_analysis_backtest', '', 'TurnPlan · 两轮 · 分析+回测复合', '独立 session：首轮应澄清分析/回测优先级；用户选择「个股/指标分析」后应分析中际旭创。', '["新 session：运行前清空 Dock Chat","首轮发送：「帮我把中际旭创分析一下，然后再跑个回测看看效果」","若 Agent 展示澄清选项，用户选择：「个股/指标分析」","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"帮我把中际旭创分析一下，然后再跑个回测看看效果","expect_domain":"ambiguous","expect_mode":"clarify","expect_sop":false,"forbid_tools":["run_strategy_backtest"],"clarify_reply":"个股/指标分析","min_reply_chars":20,"turn_id":"compound_analysis_backtest","dialogue":[{"role":"user","text":"帮我把中际旭创分析一下，然后再跑个回测看看效果"},{"role":"user","text":"个股/指标分析","judge":true,"on_clarify":true}],"expect_reply":{"rubric":"用户已选择「个股/指标分析」后，应针对中际旭创给出分析或行情解读，不应未经确认直接跑回测。","must_cover":["中际"],"must_not":["开始回测"]},"expect_intent":{"domain":"ambiguous","mode":"clarify","sop":false},"expect_execution":{"profile":"","forbid_tools":["run_strategy_backtest"]},"expect_routing":{"domain":"ambiguous","mode":"clarify","sop":false,"forbid_tools":["run_strategy_backtest"]},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7}}', 63, 1, datetime('now'), datetime('now'));
INSERT OR IGNORE INTO agent_eval_cases (id, user_id, title, description, steps_json, supports_random_stock, options_json, sort_order, enabled, created_at, updated_at) VALUES ('turn_plan_stock_quote_ambiguous', '', 'TurnPlan · 两轮 · 股价灰区', '独立 session：首轮应澄清现价/走势；用户选择「只要当前价」后应报腾讯现价。', '["新 session：运行前清空 Dock Chat","首轮发送：「腾讯股价怎么样」","若 Agent 展示澄清选项，用户选择：「只要当前价」","verify：校验路由/工具/回复关键词 + LLM 语义评判"]', 0, '{"category":"turn_plan","plan_only":false,"session_cleanup":"before_run","dual_model_eval":false,"message":"腾讯股价怎么样","expect_domain":"ambiguous","expect_mode":"clarify","expect_sop":false,"forbid_tools":["run_strategy_backtest","get_mcp_analysis"],"clarify_reply":"只要当前价","execution_profile":"stock_analysis.price_snapshot","min_reply_chars":20,"turn_id":"stock_quote_ambiguous","dialogue":[{"role":"user","text":"腾讯股价怎么样"},{"role":"user","text":"只要当前价","judge":true,"on_clarify":true}],"expect_reply":{"rubric":"用户已选择「只要当前价」后，应给出腾讯控股的现价或简要行情（价格、涨跌幅等），而非展开 MCP 深度分析或回测。","must_cover":["腾讯"],"must_not":["开始回测"]},"expect_intent":{"domain":"ambiguous","mode":"clarify","sop":false},"expect_execution":{"profile":"stock_analysis.price_snapshot","forbid_tools":["run_strategy_backtest","get_mcp_analysis"]},"expect_routing":{"domain":"ambiguous","mode":"clarify","sop":false,"forbid_tools":["run_strategy_backtest","get_mcp_analysis"]},"judge":{"enabled":true,"model_slot":"auxiliary","min_score":0.7}}', 64, 1, datetime('now'), datetime('now'));
