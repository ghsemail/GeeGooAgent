...
func defaultTurnPlanLiveCases() []TurnPlanLiveCase {
	return []TurnPlanLiveCase{
		...
		{
			ID: "stock_technical_chain", Category: TurnPlanCatStockAnalysis, Title: "多轮 · 查价后看技术面",
		...
		// ── 灰区 / 澄清 ──
		{
			ID: "ambiguous_bare_macd", Category: TurnPlanCatClarify, Title: "两轮 · 模糊 MACD",
			Description: "独立 session：首轮应 clarify；用户选择「先问答」后应讲解 MACD 用法。",
		...
		},
		{
			...
			Description: "独立 session：首轮应澄清分析/回测优先级；用户选择「个股/指标分析」后应分析中际旭创。",
		...
		},
		{
			...
			Description: "独立 session：首轮应澄清现价/走势；用户选择「只要当前价」后应报腾讯现价。",
			Message:     "腾讯股价怎么样",
			ClarifyReply: "只要当前价",
			ExpectDomain: "ambiguous", ExpectMode: "clarify", ExpectSOP: false,
			ExecutionProfile: "stock_analysis.price_snapshot",
