...
func encodeWorking(w *PreMarketWorking) map[string]any {
	...
	for k, v := range w.Stocks {
		stocks[k] = map[string]any{
			...
			"premarket_market_reason": v.PreMarketReason, "premarket_market_suggestion": v.PreMarketSuggestion,
			"stock_premarket_report_id": v.PreMarketReportID,
			"report_summary": v.ReportSummary, "report_market_summary": v.ReportMarketSummary,
			"report_trade_summary": v.ReportTradeSummary, "report_experience_summary": v.ReportExperienceSummary,
		...
		for code, raw := range stocks {
			if m, ok := raw.(map[string]any); ok {
				w.Stocks[code] = StockWorkspace{
					...
					PreMarketReason: str(m, "premarket_market_reason"), PreMarketSuggestion: str(m, "premarket_market_suggestion"),
					PreMarketReportID: str(m, "stock_premarket_report_id"),
					ReportSummary: str(m, "report_summary"), ReportMarketSummary: str(m, "report_market_summary"),
					ReportTradeSummary: str(m, "report_trade_summary"), ReportExperienceSummary: str(m, "report_experience_summary"),
