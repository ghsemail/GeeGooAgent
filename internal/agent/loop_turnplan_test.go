...
func TestAgentRoutesEmitsPlanStyle(t *testing.T) {
	...
	if planStyle != "agent_routes" {
		t.Fatalf("plan_style=%q", planStyle)
	}
...
func TestAgentRoutesDoesNotSetClassifierDomain(t *testing.T) {
	provider := &llm.MockProvider{
		Responses: []*llm.Response{
			{Content: "腾讯技术面偏强。"},
			{Content: "茅台继续观察。"},
	...
	if len(styles) != 2 || styles[0] != "agent_routes" || styles[1] != "agent_routes" {
		t.Fatalf("plan styles=%v", styles)
	}
	if session.LastTurnDomain != "" {
		t.Fatalf("LastTurnDomain should stay empty without preset plan, got %q", session.LastTurnDomain)
	}
}
...
func TestTurnPlanExecutesBacktestViaReAct(t *testing.T) {
	...
	session := runtime.NewSession()
	// Single strategy keyword avoids multi-strategy workflow (SAR+MACD would hijack the turn).
	result := loop.RunTurn(context.Background(), session, "帮我回测小米 MACD 信号", tools.Context{}, nil)
	if result.Failed {
		t.Fatalf("failed: %s", result.Error)
	}
...
func TestToolFirstSkipEmitsGateEvent(t *testing.T) {
	...
	session := runtime.NewSession()
	_ = loop.RunTurn(context.Background(), session, "帮我回测小米 MACD", tools.Context{}, nil)
	if !strings.HasPrefix(gateDecision, "skip") {
		t.Fatalf("expected skip gate, got %q", gateDecision)
	}
}

func TestAmbiguousTurnUsesClarifyTool(t *testing.T) {
	registry := tools.NewRegistry()
	tools.RegisterClarifyTool(registry)
	gateMock := &llm.MockProvider{
		Responses: []*llm.Response{{Content: `{"retrieve":false,"query":"","reason":"test"}`}},
	}
	provider := &llm.MockProvider{
		Responses: []*llm.Response{
			{ToolCalls: []llm.ToolCall{{ID: "c1", Name: "clarify", Arguments: map[string]any{
				"question": "你是想做哪一件？",
				"choices":  []any{"个股/指标分析", "查实时股价", "信号/策略", "策略回测"},
			}}}},
			{Content: "好的，继续分析。"},
		},
	}
	...
	session := runtime.NewSession()
	result := loop.RunTurn(context.Background(), session, "MACD", toolCtx, registry.Schemas([]string{"clarify"}))
	if result.Failed {
		t.Fatalf("failed: %s", result.Error)
	}
	sawClarifyTool := false
	...
	if !sawClarifyTool {
		t.Fatalf("expected clarify tool_start in %v", events)
	}
