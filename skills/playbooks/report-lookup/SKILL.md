...
3. **今日盘后** — `list_today_post_market_reports`
4. **单类列表** — `get_pre_market_reports` / `get_intraday_reports` / `get_post_market_reports`
...
| 「帮我跑一遍盘前自动化」 | `geegoo run pre_market` / scheduler（**不要**在 chat 里乱调 `create_pre_market_report`） |
