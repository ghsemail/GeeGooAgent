...
def search_news(query, size=10):
    ...
    data = {"query": query, "variables": 0, "size": size}
    resp = requests.post(API_URL, headers=headers, json=data, timeout=30)
    result = resp.json()
    if result.get('status') != 0 and result.get('success') is not True:
        return {'error': result.get('message', 'API Error')}
    articles = result.get('data',{}).get('data',{}).get('llmSearchResponse',{}).get('data',[])
    return [{'title': a.get('title',''), 'content': a.get('content','')[:200], 'source': a.get('source',''), 'date': a.get('date','')} for a in articles]
