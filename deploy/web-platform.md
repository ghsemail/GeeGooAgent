...
GeeGooBot `agent-api`（118.195.135.97）需能访问 GeeGooAgent `agent-runtime`（119.45.16.112:3400）。

在 **119.45.16.112 腾讯云安全组** 放行 **TCP 3400**，来源建议：

- `118.195.135.97/32`（GeeGooBot BFF）
- 运营 Web 服务器 IP（若浏览器直连 runtime 调试）

验证（在 Bot 机执行）：

```bash
curl -sf --max-time 5 http://119.45.16.112:3400/health
```

若超时，BFF 会返回 `502 agent-runtime unreachable`（与 Bearer/MCP 无关）。
