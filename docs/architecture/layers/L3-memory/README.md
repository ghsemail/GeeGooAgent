...
> **Memory port**：`memport.Port`（`Recall` / `Store` / `Compress`）；`memory.Adapter` 委托现有实现。  
> **Session SSOT**：`chatsession`（SQLite）；port 不写 conversation 真相源。  
> **Recall 排序**：Adapter `SessionRanker` → `agent.RankRecallHits` → cognition Ranker。
