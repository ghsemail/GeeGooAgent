...
```text
Tools
  ├── MarketClient     → :5700  (sk-...)   geegoo mcp workflow
  ├── GeeGooBotClient    → :5700  (sk-...)   geegoo 主 API
  └── SignalClient     → :5800             信号指标（可选）
```
...
| `/getStockDailyReports` | —                                  | 5700 正常 | **按日期聚合** pre/intraday/post 时优先用此接口                 |
