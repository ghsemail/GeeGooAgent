...
| ReAct 循环 | `internal/runtime/react.go` — `ReActLoop.RunTurn` |
| Tool 执行 | `internal/runtime/executor.go` |
...
RunTurn(ctx, session, userText, toolCtx, schemas)
  ...
  └─ loop (max_tool_rounds):
        ...
        └─ 有 tool_calls:
              for each call:
                Progress.Emit("tool", name)
                Registry.Execute(call, toolCtx)
...
- EventBus（L0）— workflow 路径发 `ToolCalled` / `ToolCompleted`
...
- `internal/runtime/react_test.go`
- `internal/runtime/react_interrupt_test.go`
- `internal/runtime/react_compress_test.go`
