...
curl -X POST "http://localhost:5700/getSinglePromptTemplate" \
...
curl -X POST "http://localhost:5700/getMCPAnalysis" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <API_KEY>" \
...
```bash
curl -X POST "http://localhost:5700/getStockDailyReports" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <API_KEY>" \
